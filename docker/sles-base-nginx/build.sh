...
# liblmdb-devel package is not available in CBOS, on build time installing liblmdb-devel from liblmdb.src
zypper in -y rpm-build autoconf automake libtool liblmdb-0_9_30 &&\
zypper download lmdb.src &&\
rpmbuild --rebuild $(find /var/cache/zypp/packages -name "lmdb*.src.rpm" 2>/dev/null)
rpm -i $(find /usr/src/packages/RPMS/x86_64/ -name "lmdb-devel*" 2>/dev/null)
...
useradd www-data -r -u 101 -d /usr/local/nginx -g101
